...
Server::Server(int port) : port_(port), running_(false), next_client_id_(1) {
...
//   std::cout << "Server created for port " << port_ << "." << std::endl;
  LOG_INFO("Server", "Server created for port {}.", port_);
}

Server::~Server() {
  stop();
  LOG_INFO("Server", "Server destroyed.");
}
...
void Server::start() {
  ...
  if (!listen_socket_ || !listen_socket_->bind_socket(port_)) {
    LOG_ERROR("Server", "Failed to bind socket on port {}.", port_);
    return;
  }
  ...
  while (running_) {
    ...
    if (!client_socket || !client_socket->is_valid()) {
      if (running_) {  // Only log error if we are supposed to be running
        ...
        // std::cerr << "Server: Failed to accept new connection or server shutting down." <<
        // std::endl;
      }
    ...
    auto client_handler = std::make_unique<ClientHandler>(client_id, std::move(client_socket),
                                                          *this, default_message_handler_);
    ...
    common::Message join_msg(common::MessageType::CLIENT_JOINED, 0, 0,
                             "Client " + std::to_string(client_id) + " joined.");
...
void Server::broadcast_message(const common::Message& msg, uint32_t sender_id_to_exclude) {
  std::lock_guard<std::mutex> lock(clients_mutex_);
  // std::cout << "Server broadcasting message from " << msg.header.sender_id << " (excluding " <<
  // sender_id_to_exclude << ")" << std::endl;
...
void Server::cleanup_clients() {
  ...
  while (running_) {
    ...
    finished_clients_cv_.wait(lock, [this] { return !running_ || !finished_client_ids_.empty(); });
...
void Server::remove_client(uint32_t client_id) {
  ...
  {
    std::lock_guard<std::mutex> lock(clients_mutex_);
    auto it = std::find_if(clients_.begin(), clients_.end(), [client_id](const auto& handler_ptr) {
    ...
    if (it != clients_.end()) {
      ...
      std::cout << "Server: Client " << client_id
                << " not found for removal (possibly already removed)." << std::endl;
    }
  ...
  if (handler_to_delete) {
    ...
    std::cout << "Server: ClientHandler for " << client_id << " stopped and resources released."
              << std::endl;

    // Notify other clients about the departure (optional)
    common::Message leave_msg(common::MessageType::CLIENT_LEFT, 0, 0,
                              "Client " + std::to_string(client_id) + " left.");
